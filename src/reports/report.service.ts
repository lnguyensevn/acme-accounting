...
import { AccountingReportError } from '../common/error';
  ...
  async generate(filter: { types: ReportType[] }): Promise<void> {
    try {
      if (filter.types.length === 0) {
        throw new AccountingReportError('No report types specified');
      }
      if (this.isGenerating) {
        throw new AccountingReportError(
      ...
      if (!this.cache?.time) {
        ...
        this.cache.setCacheTime(new Date());
      }
      ...
      if (filter.types.includes(ReportType.accounts)) {
        ...
        this.cache.setStates(ReportType.accounts, {
          ...
          metrics: {
            ...
            dataWriteTimeTaken: accountReport.writeTimeTaken,
      ...
      if (filter.types.includes(ReportType.yearly)) {
        ...
        this.cache.setStates(ReportType.yearly, {
          ...
          metrics: {
            dataFetchingTimeTaken: dataFetchTimeTaken,
            dataWriteTimeTaken: yearlyReport.writeTimeTaken,
      ...
      if (filter.types.includes(ReportType.fs)) {
        ...
        this.cache.setStates(ReportType.fs, {
          ...
          metrics: {
            dataFetchingTimeTaken: dataFetchTimeTaken,
            dataWriteTimeTaken: fs.writeTimeTaken,
